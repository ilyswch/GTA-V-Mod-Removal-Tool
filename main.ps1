...
$folder = Read-Host -Prompt "Enter the path of your GTA V Installation (e.g. C:\Program Files (x86)\Steam\steamapps\common\Grand Theft Auto V)"

if ([string]::IsNullOrWhiteSpace($folder)) {
    Write-Host "Error: No path entered. Please restart the script and enter a valid path." -ForegroundColor Red
    Read-Host
    exit
}

Get-ChildItem -Path $folder -Exclude $excluded | Remove-Item -Recurse -ErrorAction Stop
